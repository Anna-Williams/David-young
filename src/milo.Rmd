...
het <- ggplot(da_results, aes(PValue)) + geom_histogram(bins=100) + scale_y_continuous(limits = c(0,700)) + ggtitle(contrastHET) + geom_abline(intercept=dim((da_results))[1]/100)
da_results_ko <- testNhoods(milo, design = ~ 0  + genotype + batch, design.df = design, model.contrasts = contrastKO, reduced.dim = "corrected")
ko <- ggplot(da_results_ko, aes(PValue)) + geom_histogram(bins=100) + ggtitle(contrastKO) + geom_abline(intercept=dim((da_results))[1]/100)
