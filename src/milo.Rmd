...
```{r}
design <- data.frame(colData(milo))[,c("Sample", "batch", "genotype")]
...
```{r test-da}
# we need to use the ~ 0 + Variable expression here so that we have all of the levels of our variable as separate columns in our model matrix
da_result <- testNhoods(milo, design = ~ 0  + genotype + batch, design.df = design, model.contrasts = contrastKO, reduced.dim = "corrected")
...
```{r}
ggplot(da_results, aes(PValue)) + geom_histogram(bins=50) + ggtitle(contrastKO)
```
...
```{r test-da}
# we need to use the ~ 0 + Variable expression here so that we have all of the levels of our variable as separate columns in our model matrix
da_results <- testNhoods(milo, design = ~ 0  + genotype + batch, design.df = design, model.contrasts = contrastHET, reduced.dim = "corrected")
table(da_results$SpatialFDR < 0.1)
```
This calculates a Fold-change and corrected P-value for each neighbourhood, which indicates whether there is significant differential abundance between developmental stages. The main statistics we consider here are: 
...
```{r}
ggplot(da_results, aes(PValue)) + geom_histogram(bins=50) + scale_y_continuous(limits = c(0,700)) + ggtitle(contrastHET) 
```


#### Test the other way arround
We observe all are NS, with a slightly conservative tendency. Is this due to not correct design?  test WTvsHET to compare.
```{r test-da-het}
# we need to use the ~ 0 + Variable expression here so that we have all of the levels of our variable as separate columns in our model matrix
da_results_rev <- testNhoods(milo, design = ~ 0 + genotype, design.df = design, model.contrasts = "genotypeWT - genotypeHET", reduced.dim = "corrected")
table(da_results$SpatialFDR < 0.1)
```

```{r}
ggplot(da_results_rev, aes(PValue)) + geom_histogram(bins=50) + ggtitle("genotypeWT - genotypeHET") + scale_y_continuous(limits = c(0,700)) + abline(y=100)
```

The same pattern is still observed. 

### how about removing nhoods that are only in KO?
...
het <- ggplot(da_results_het, aes(PValue)) + geom_histogram(bins=50) + scale_y_continuous(limits = c(0,700)) + ggtitle(contrastHET) + geom_abline(intercept=dim((da_results_het))[1]/50)
da_results_ko <- testNhoods(milo, design = ~ 0  + genotype + batch, design.df = design, model.contrasts = contrastKO, reduced.dim = "corrected")
ko <- ggplot(da_results_ko, aes(PValue)) + geom_histogram(bins=50) + ggtitle(contrastKO) + geom_abline(intercept=dim((da_results))[1]/50)
ggplot(da_results_het, aes(PValue)) + geom_histogram(bins=500) + ggtitle(contrastHET) + geom_abline(intercept=dim((da_results_het))[1]/500)
gridExtra::grid.arrange(het,ko)
