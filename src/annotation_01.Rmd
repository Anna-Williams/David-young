---
title: "Annotation cluster res06"
...
<!-- ```{r top_genes} -->
<!-- if(!file.exists(here("outs", project, "top_genes_res06_01.tsv"))){ -->
<!-- # Create object where instead of cells in cols we have the clusters with counts aggregated -->
<!-- summed_res06 <- aggregateAcrossCells(sce, id=colData(sce)[,c("originalexp_snn_res.0.6")]) -->

<!-- # Delete ribosomal and mitochondrial genes -->
<!-- is_ribo <- grepl("^Rp[sl]", rownames(summed_res06)) -->
<!-- is_mito <- grepl("^mt-", rownames(summed_res06)) -->
<!-- keep <- !(is_ribo | is_mito) -->
<!--  summed_res06 <- summed_res06[keep,] -->

<!-- # for each column (aggregated counts) of the object extract the top genes -->
<!-- top_genes_res06 <- apply(counts(summed_res06),  -->
<!--       MARGIN = 2, -->
<!--       function(x){ -->
...
<!--       ) -->
<!-- # save result -->
<!-- write.table(top_genes_res06, file = here("outs", project, "top_genes_res06_01.tsv"), sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE) -->
<!-- } -->
<!-- ``` -->

<!-- We compute here a pairwise differential expression between all the clusters. The results are saved in a list of dataframes, "markers_res06_01/markers_res06_01.RDS" -->
<!-- one for each cluster. A .csv file (that can be opened with a spreadsheet program) for each cluster is available in "markers_res06_01". -->
...
<!-- ```{r markers} -->
<!-- # compute markers -->
<!-- if(!file.exists(here("outs", project, "markers_res06_01", "markers_res06_01.RDS"))){ -->
<!--   dir.create(here("outs", project, "markers_res06_01")) -->
<!--   markers <- findMarkers(sce, groups = sce$originalexp_snn_res.0.6, direction="up", lfc=1) -->
<!--   saveRDS(markers, here("outs", project,"markers_res06_01", "markers_res06_01.RDS")) -->
<!--   # save the top 100 genes for each one of the df -->
<!--   lapply(names(markers), function(x){ -->
<!--     top_markers <- head(markers[[x]], 100) -->
<!--     write.csv(top_markers, here("outs", project, "markers_res06_01", paste0("top_markers_", x, "res06_01.csv")), quote = FALSE) -->
<!--     } -->
<!--   ) -->
<!-- } -->
<!-- ``` -->
