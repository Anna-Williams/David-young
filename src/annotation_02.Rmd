---
title: "Annotation cluster res 0.4"
...
```{r top_genes}
if(!file.exists(here("outs", project, "top_genes_res0.4.tsv"))){
...
# save result
write.table(top_genes_res0.4, file = here("outs", project, "top_genes_res0.4.tsv"), sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)
}
```
...
```{r markers}
# compute markers
if(!file.exists(here("outs", project, "markers_res0.4", "markers_res0.4.RDS"))){
  dir.create(here("outs", project, "markers_res0.4"))
  markers <- findMarkers(sce, groups = sce$originalexp_snn_res.0.4, direction="up", lfc=1)
  saveRDS(markers, here("outs", project,"markers_res0.4", "markers_res0.4.RDS"))
  # save the top 100 genes for each one of the df
  lapply(names(markers), function(x){
    top_markers <- head(markers[[x]], 100)
    write.csv(top_markers, here("outs", project, "markers_res0.4", paste0("top_markers_", x, "_res0.4.csv")), quote = FALSE)
    }
  )
}
```
...
```{r result}
if (!file.exists(here("processed", project, "sce_anno_02.RDS"))) {
annotation <- read.delim(here("data", "young_FIRE_mice_cluster_anno_02_res.0.4.tsv"))
...
sce$clusters_named <-
  factor(sce$clusters_named,
    levels = c("Microglia", "BAMs","Immune", "Astro_1", "Astro_2", "Astro_Oligo", "OPC", "pOPC", "iOligo", "mOligo_1", "mOligo_2", "mOligo_3", 
"mOligo_4", "Endothelial", "Mural_cells", "ChP_epithelia", "iNeurons_&_NRPs", "mNeuron_ex",  
 "mNeuron_in"
 )
  )

# add annotation of the samples (cool way to map, thanks stackoverflow)
sample_named_map <- c("WTyo1_h", "HETyo2_h", "KOyo3_h", "WTyo1_c", "HETyo2_c", "KOyo3_c", "WTyo4_h", "HETyo5_h", "KOyo6_h","WTyo4_c", "HETyo5_c", "KOyo6_c")
names(sample_named_map) <- unique(sce$Sample)

sce$sample_named <- factor(sample_named_map[sce$Sample], levels = c("WTyo1_h", "HETyo2_h", "KOyo3_h", "WTyo1_c", "HETyo2_c", "KOyo3_c", "WTyo4_h", "HETyo5_h", "KOyo6_h","WTyo4_c", "HETyo5_c", "KOyo6_c"))
