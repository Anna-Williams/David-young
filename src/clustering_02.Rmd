...
sce_anno <- readRDS(here("processed/fire-mice/sce_corrected_02.RDS"))
celltype <- colData(sce_anno)["celltype"]
sce_anno <- readRDS(here("processed/fire-mice/sce_clusterQC.RDS"))
celltype <- colData(sce_anno)["celltype"]
identical(row.names(colData(sce)), row.names(celltype))
sce$celltype <- celltype$celltype
